...
COPY --from=builder /www/packages/app/.next/standalone/ .
COPY --from=builder /www/packages/app/.next/static/ ./packages/app/.next/static/
...
RUN chown -R ${USER_NAME}:${USER_GROUP_NAME} ./packages/app
